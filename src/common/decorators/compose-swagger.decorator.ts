...
export function GetAllBoardSwagger() {
  return applyDecorators(
    ...
    ApiParam({ name: 'param', type: SelectBoardDto }),
    ApiResponse({ status: 200, description: 'success', type: BoardEntity, isArray: true }),
  );
}

export function GetOneBoardSwagger() {
  return applyDecorators(
    ApiOperation({ summary: '게시글 단건 조회', description: '게시글 조회' }),
    ApiResponse({ status: 200, description: 'success', type: BoardEntity, isArray: true }),
  );
}
...
export function DeleteCommentSwagger() {
  return applyDecorators(
    ApiOperation({ summary: '한 개의 댓글 삭제', description: '한개 댓글을 삭제합니다.' }),
  );
}
...
export function PostSignInSwagger() {
  return applyDecorators(
    ...
    ApiBody({ type: CurrentUserDto, description: '로그인/회원가입 필수 정보', required: true }),
    ApiResponse({
      schema: {
        example: {
...
export function GetBoardsInCategorySwagger() {
  return applyDecorators(
    ApiOperation({
      ...
      description: '각 카테고리별 게시글들을 최신순으로 호출, 아직 필터(정렬) 없음',
    }),
  );
}
